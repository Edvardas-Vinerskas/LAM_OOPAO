...
env = OOPAO_environment_ZWFS_1_stage()
...
iters                = 6 #TODO change how you plot the strehl ratio to include warmup
episode_length       = 500
initial_sigma        = 0.1
min_sigma            = 0
warmup_episodes      = 10
...
def train_policy(optimizer: SharedAdam, policy: ConvPolicyFastFast, dynamics: EnsembleDynamicsFast,
    ...
    print("ATTENTION: policy in training")
...
def run_episode_policy(past_obs, past_act, obs, replay, policy, sigma, episode_length):
    ...
    for t in range(episode_length):
        ...
        next_obs, INFO = env.step(action.squeeze())
...
def run_episode_warmup(replay, replay_warmup, sigma, episode_length, filter, filter_pyr, xvalid, yvalid):
    ...
    for t in range(episode_length):
        ...
        tracker = INFO["tracker"]
        print(f"warmup strehl 1: {strehl_1}; atm tracker: {tracker}")

        past_obs = torch.cat([past_obs[:, 1:, :, :], obs.unsqueeze(0).unsqueeze(0).to(device0)], dim=1)  # roll telemetry
        past_act = torch.cat([past_act[:, 1:, :, :], action], dim=1)
...
def main():
    #the 2 buffers should never be the same, since you might overwrite the load_buffer
    directory_name = 'test_ZWFS_single_stage_noiseless' #should train on seed 5 and continue evaluating on seed 5 (noiseless)
    directory_name_load_buffer = 'test_PWFS_noise_cpu_seed_chang'
    ...
    if load_buffer == False:
        for i in range(warmup_episodes): #warmup_episodes
            #obs, _ = env.reset(seed=np.random.randint(11, 256))
            start = time.time()
            #-----------------------------------sample generation-----------------------------------#
            reward_sum, past_obs, past_act, obs, strehl_warmup = run_episode_warmup(replay, replay_warmup, sigma, episode_length, KL_projection.to(device0), KL_projection, xvalid0, yvalid0)
    ...
    if load_buffer:
        ...
        replay_warmup.set_len(warmup_episodes * episode_length - 1)
        ...
        replay.set_len(warmup_episodes * episode_length - 1)
        print(
