...
class EfficientExperienceReplay():
    ...
    '''def sample_contiguous(self, horizon, max_ts, batch_size=32):
        device = self.states.device  # already cuda:0, keep everything there

        # Generate start indices directly on GPU — no CPU→GPU transfer
        episode_starts = torch.randint(0, len(self) // max_ts, size=(batch_size,), device=device) * max_ts
        within_episode = torch.randint(0, max_ts - (horizon + 1), size=(batch_size,), device=device)
        inds = episode_starts + within_episode  # [batch_size]

        # Single vectorized arange instead of batch_size individual ones
        offsets = torch.arange(horizon + 1, device=device)          # [horizon+1]
        indices = inds.unsqueeze(1) + offsets.unsqueeze(0)           # [batch_size, horizon+1]
        indices = indices.flatten()                                   # [batch_size*(horizon+1)]
