...
class ZWFS2:
    ...
    def reconstructor(self, reconstructor = 'atan', iteration = 1, damping_iteration = 1):
        self.zwfs1.reset_Ib_beta()
        self.zwfs2.reset_Ib_beta()
        self.zwfs1.sin_phase = self.zwfs1.phase_sin(clipping=False)
        self.zwfs2.sin_phase = self.zwfs2.phase_sin(clipping=False)
        ...
        for i in range(iteration):
            ...
            bad = np.abs(self.det_M) < 1e-5
            
            if (bad.any() & (self.det_M[self.zwfs1.telescope.pupil==1]==0)).sum()>0:
            ...
            if iteration>1:
                ...
                phase1[self.zwfs1.telescope.pupil==1] =(1-damping_iteration)*phase1[self.zwfs1.telescope.pupil==1] +damping_iteration*retrieved_phase[self.zwfs1.pupil_mask]
                # phase1[np.where(~np.isfinite(phase1))]=0
                
                phase2[self.zwfs2.telescope.pupil==1] = (1-damping_iteration)*phase2[self.zwfs2.telescope.pupil==1] +damping_iteration*retrieved_phase[self.zwfs2.pupil_mask]
