...
from po4ao_models_upd import ConvDynamicsFast, EnsembleDynamicsFast, ConvPolicyFastFast
...
#TODO you didn't save atm KL modes
#TODO you didn't save the background frames either my guy
...
#TODO test this to be a 2D mask
...
## observation and dm buffer
frame_data_1st_stage = dao.shm('/tmp/papyrus_res_wf.im.shm')  # Dao Memory Camera #is this the current equivalent? SHM_RESIDUAL_WF = "/tmp/resWf.im.shm"
...
frame_data_2nd_stage = dao.shm('/tmp/oziriis_res_wf.im.shm')  # Dao Memory Camera #is this the current equivalent? SHM_RESIDUAL_WF = "/tmp/resWf.im.shm" 
#no frame counter in pixel 00
dm_shm_2nd_stage = dao.shm('/tmp/dm2ndStageCmd01.im.shm')
...
'''#TODO the action here is just gain * next_obs
@torch.no_grad()
def step_1st_stage(action):
    """
    Pipeline specific function that sends new commands to dm, i.e., sets the action, and reads the
    following WFS measurement projected to DM space trought a linear recontructor. The action and
    the WFS measurement have to be 2D images.
    ...
    # dm[:] = prev_commands - (dm_image_torch * gain)
    temp = (prev_commands_1st_stage * config['integrator']['leak']) + (action.squeeze())
    prev_commands_1st_stage[:,:] = temp.clamp(-0.8,0.8) #clipping limits

    # Calculations are done, move results into the correct SHM buffer
    dm_shm_1st_stage.set_data(temp[dm_coords_1st_stage].cpu().numpy())
...
def train_policy(optimizer: SharedAdam, policy: ConvPolicyFastFast, dynamics: EnsembleDynamicsFast, replay: EfficientExperienceReplay,
    ...
    for i in range(pol_iters):
        ...
        for t in range(0, planning_horizon):
            ...
            losses += loss_fn(torch.mean(next_state, dim = 1), action)
...
def training_thread(start_q,dynamics_q, dynamics_optimizer_q, replay_q, replay_warmup_q, policy_optimizer_q, policy_q, finished_q, dyn_iters = 30, pol_iters = 10):
    ...
    while(1):
        ...
        if start:
            ...
            dyn_loss = train_dynamics(dynamics, dynamics_optimizer, replay, replay_warmup, dyn_iters=dyn_iters)
            torch.cuda.synchronize(device=device1)

            pol_loss = train_policy(policy_optimizer, policy, dynamics, replay, replay_warmup, pol_iters=pol_iters)
            torch.cuda.synchronize(device=device1)
...
def run_episode_policy(past_obs, past_act, obs, replay, policy, sigma, episode_length):
    ...
    for t in range(episode_length):
        ...
        next_obs_1st = frame_data_1st_stage.get_data(check = True).astype(np.float32)
        ...
        next_obs_1st_list.append(next_obs_1st)

        obs = next_obs
        
    return reward_sum, past_obs, past_act, obs, next_obs_1st_list
...
def run_episode_warmup(replay, replay_warmup, sigma, episode_length, filter, xvalid, yvalid):
    ...
    subprocess.Popen(['python', '/home/daouser/dao/daopapyrus-dev/evinerskas/1s_stage_noise_add.py', str(int(episode_length)), str(sigma * 5)]) #start the noise 
    ...
    #this is for 1st stage convergence
    counter = 0
    while (counter < 15):
        frame_data_1st_stage.get_data(check = True)
        counter += 1
...
def main():
    ...
    if config['save_and_load']['load_models_pretrained']:
        ...
        np.save(f"{savedir}/dynamics_loss_warmup", dynamics_loss)
        np.save(f"{savedir}/policy_loss_warmup", policy_loss)
    ...
    if replay.len > episode_length and replay_warmup.len > episode_length:
        training_process = ctx.Process(target=training_thread, args=(start_q,dynamics, dynamics_optimizer, replay_q, replay_warmup_q, policy_optimizer, policy, finished_q, config['training']['dynamics_grad_steps'], config['training']['policy_grad_steps'],))
    ...
    for i in range(iters):
        ...
        reward_sum, past_obs, past_act, obs, next_obs_1st_list = run_episode_policy(past_obs, past_act, obs, replay, policy_copy, sigma,
                                                                 episode_length)

        rewards[i + warmup_episodes] = reward_sum

        all_next_obs_1st.extend(next_obs_1st_list)
    ...
    #np.save(os.path.join(savedir, f"papy_modes.npy"), np.asarray(dao.shm('/tmp/papyrus_modes.im.shm').get_data().astype(np.float32)))
    np.save(os.path.join(savedir, f"next_states_1st.npy"), np.asarray(all_next_obs_1st))
    torch.save(rewards, os.path.join(savedir, f"rewards.pt"))
    torch.save(replay.states, os.path.join(savedir, f"states.pt"))
    torch.save(replay.actions, os.path.join(savedir, f"actions.pt"))
    torch.save(replay.next_states, os.path.join(savedir, f"next_states_2nd.pt"))
