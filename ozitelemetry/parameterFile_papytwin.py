...
def initializeParameterFile():
    ...
    param['r0'                   ] = 0.04                                       # value of r0 in the visibile in [m]
    param['L0'                   ] = 30                                             # value of L0 in the visibile in [m]
    param['fractionnalR0'        ] = np.array([0.45,0.1,0.1,0.25,0.1]).astype(np.float64)                       # Cn2 profile
    param['V0'                   ] = 3
    param['windSpeed'            ] = np.array([5,4,8,10,2]        ).astype(np.float64)                        # wind speed of the different layers in [m.s-1]
    param['windSpeed'            ] *= param['V0'                   ]/((np.sum(np.asarray(param['fractionnalR0']) * np.asarray(param['windSpeed'])))**(5/3))**(3/5)
