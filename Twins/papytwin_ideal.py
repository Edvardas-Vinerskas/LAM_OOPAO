...
display                 = False
frame_delay             = 0
wfsSignal               = np.zeros((nLoop,wfs.nSignal))

reconstructor = M2C_CL@calib_CL.M
reconstructed_cmd = np.zeros((nLoop,M2C.shape[0]))
...
for i in range(nLoop):
    ...
    wfsSignal[k+frame_delay]=wfs.signal
    reconstructed_cmd[k] = np.matmul(reconstructor,wfsSignal[k])*1/0.65
    # apply the commands on the DM
