"""
the step function action is not masked? (so is it masked before the input)
    because I mask the action inside my step function
    no they are masked using dm_coords
...
m2v_data = dao.shm('/tmp/papyrus_M2C.im.shm').get_data()

valid_mask = dao.shm('/tmp/dm241Map.im.shm').get_data()
dm_x, dm_y = np.where(valid_mask)
dm_coords = (dm_x, dm_y)

## observation and dm buffer
frame_data = dao.shm('/tmp/papyrus_res_wf.im.shm')  # Dao Memory Camera
dm_shm = dao.shm('/tmp/dmCmd03.im.shm')

## DAO GAIN
# gain=dao.shm('/tmp/lpGain.im.shm')
# leak = dao.shm('/tmp/lpLeak.im.shm')

prev_commands = torch.as_tensor(np.zeros((17, 17)), device=device0).float()
obs_image = torch.as_tensor(np.zeros((17, 17)), device=device0).float()


@torch.no_grad()
def sample_noise(sigma, flt, xvalid, yvalid):
    action_vec = torch.matmul(flt, sigma * torch.sign(torch.randn((241,)).to(device0)))
    action_im = torch.zeros((17, 17)).to(device0).float()
...
def step(action):
    ...
    temp = (prev_commands * 0.99) + (action.squeeze())
    prev_commands[:,:] = temp.clamp(-0.8,0.8) #clipping limits
...
def flatten_dm():
    ...
    """
    dm_shm.set_data(prev_commands[dm_coords].cpu().numpy()*0)
...
def main():
    ...
    savedir = f'~/PO4AO/logs/{timestamp}'
    ...
    for i in range(iters):
        ...
        reward_sum, past_obs, past_act, obs = run_episode_policy(past_obs, past_act, obs, replay, policy_copy, sigma,
                                                                 episode_length)
