...
m2v_data_2nd_stage = dao.shm('/tmp/oriziis_M2C.im.shm').get_data().astype(np.float32)

valid_mask_2nd_stage = dao.shm('/tmp/dm97Map.im.shm').get_data()
dm_x_2nd_stage, dm_y_2nd_stage = np.where(valid_mask_2nd_stage)
dm_coords_2nd_stage = (dm_x_2nd_stage, dm_y_2nd_stage)

## 1st stage dm closed loop channel
...
prev_commands_2nd_stage = torch.as_tensor(np.zeros((data_shape_2nd_stage, data_shape_2nd_stage)), device=device0).float()
obs_image_2nd_stage = torch.as_tensor(np.zeros((data_shape_2nd_stage, data_shape_2nd_stage)), device=device0).float()


@torch.no_grad()
def sample_noise(sigma, flt, xvalid, yvalid):
    action_vec = torch.matmul(flt, sigma * torch.sign(torch.randn((97,)).to(device0)))
...
def step_2nd_stage(action):
    ...
    dm_shm_2nd_stage.set_data(temp[dm_coords_2nd_stage].cpu().numpy())

    obs_image_2nd_stage[dm_coords_2nd_stage] = torch.from_numpy(frame_data_2nd_stage.get_data(check=True, semNb=5)).flatten().to(device0)
...
def train_dynamics(dynamics: EnsembleDynamicsFast, optimizers: list[SharedAdam], replay: EfficientExperienceReplay, replay_warmup: EfficientExperienceReplay, dyn_iters=5):
    """
    ...
    for i in range(dyn_iters):
        loss = 0
        for bs_model, opt in zip(dynamics.models, optimizers):
            opt.zero_grad()
            ...
            pred_loss.backward()
            torch.nn.utils.clip_grad_norm_(bs_model.parameters(), 0.5)
            opt.step()
...
def training_thread(start_q, dynamics_q, dynamics_optimizer_q, replay_q, replay_warmup_q, policy_optimizer_q, policy_q, finished_q, dyn_iters = 30, pol_iters = 10):
    ...
    '''print("Training process started — warming up compiled models...")

    # --- Warmup: run one dummy pass through both compiled models ---
    # Use the first real batch that was already put in the queues
    replay          = replay_q.get()
    replay_warmup   = replay_warmup_q.get()

    #with torch.no_grad():
    # One dummy forward through dynamics
    train_dynamics(dynamics_q, dynamics_optimizer_q, replay, replay_warmup, dyn_iters=5)
    torch.cuda.synchronize(device=device1)

    # One dummy forward through policy
    train_policy(policy_optimizer_q, policy_q, dynamics_q, replay, replay_warmup, pol_iters=5)
    torch.cuda.synchronize(device=device1)

    print("Warmup complete.")
    warmup_done_q.put(True)'''

    os.sched_setaffinity(0, {10, 11})  # 0 means current process
...
def run_episode_policy(past_obs, past_act, obs, replay, policy, sigma, episode_length, current_iteration):#, nxt_sts_2nd_lst, crnt_iter, ctrl_jit):
    ...
    for t in range(episode_length):
        ...
        '''if np.max(dm_shm_2nd_stage.get_data()) > 0.075:
            dm_shm_2nd_stage.set_data(dm_shm_2nd_stage.get_data() * 0)
            time.sleep(5e-3)
            print("resetting the dm")
            past_obs = past_obs_og
            past_act = past_act_og'''
...
@torch.no_grad()
def run_episode_policy_v2(past_act, obs, replay, policy, sigma, episode_length, current_iteration):#, nxt_sts_2nd_lst, crnt_iter, ctrl_jit):
    """
    runs an episode on policy
    ...
    for t in range(episode_length):
        ...
        if integrator == True or current_iteration < 20:
            ...
            input_policy = torch.cat([obs.unsqueeze(0).unsqueeze(0), past_act],dim = 1)
            action = policy(input_policy) #/10
    ...
    return reward_sum, past_act, obs
...
def run_episode_warmup_v2(replay, replay_warmup, sigma, episode_length, filter, xvalid, yvalid):
    """
    runs an episode on integrator with added noise in control signals. Starts always with a flat mirror
    ...
    obs = flatten_dm()
    #past_obs = torch.zeros(1, 1, *obs.shape, device = device0).squeeze(2) # keep telemetry in memory for the next episode
    past_act = torch.zeros(1, (n_history-1), *obs.shape, device = device0).squeeze(2)
    ...
    for t in range(episode_length):
        ...
        #past_obs = obs.unsqueeze(0).unsqueeze(0) #roll telemetry
        past_act = torch.cat([past_act[:,1:,:,:], action], dim = 1)
...
def main():
    ...
    #TODO change the *ries
    subprocesses = 420
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    #I commented out all of the 1st stage code and testing the lagging problems
    directory_name = f'onsky_Dubhe_warmup_1st200_2nd400_v4_reflattening_singleobs' #{timestamp}  #test_warmup50_1st200_2nd400_scaling1e2_noise2ndstageonly_losspenalty01_iters300_v19
    savedir = f'~2026-04-07/{directory_name}'#{timestamp}'
    loaddir = f'~2026-04-07/saved'   # copy the models and replay buffers you want use here!!
    ...
    dynamics_optimizer = [SharedAdamW(dynamics.parameters(), lr = 5e-5, weight_decay=1e-3) for model in dynamics.models]
    policy_optimizer = SharedAdamW((policy.parameters()), lr = 5e-5, weight_decay=1e-3)
    ...
    #dynamics_optimizer1 = [optim.Adam(dynamics.parameters()) for model in dynamics.models]
    #policy_optimizer1 = optim.Adam(policy.parameters())
    #dynamics_optimizer1 = optim.Adam(dynamics.parameters())
    #policy_optimizer1 = optim.Adam(policy.parameters())

    #TODO try without weight decay
    dynamics_optimizer1 = [optim.AdamW(dynamics.parameters(), weight_decay = 1e-3) for model in dynamics.models]
    ...
    if use_1st_stage:
        ...
        subprocess.Popen(['taskset', '-c', '11', 'python', '/home/daouser/dao/daopapyrus-dev/evinerskas/1s_stage_noise_add.py', str(int(episode_length/4)), str(int(warmup_episodes)), str(sigma)]) #start the noise 
    ...
    if not config['save_and_load']['load_warmup_buffer']:
        for i in range(warmup_episodes):
            start = time.time()
            reward_sum, past_obs, past_act, obs = run_episode_warmup_v2(replay, replay_warmup, sigma, episode_length, KL_projection.to(device0), xvalid0, yvalid0)
    ...
    for i in range(iters):
        ...
        reward_sum, past_obs, past_act, obs = run_episode_policy_v2(past_obs, past_act, obs, replay, policy_copy_comp, sigma,
                                                                 episode_length, current_iter)#, next_states_2nd_list, current_iter, ctrl_jitter_list)
        


        if reward_sum > arbitrary_value and i > 3:
            flatten_dm()
            past_obs = past_obs * 0
            past_act = past_act * 0
        ...
        if config['save_and_load']['save_models_pretrained_onl'] and i == 60:
    ...
    #np.save(os.path.join(savedir, f'ctrl_jitter_array.npy'), np.asarray(ctrl_jitter_list))
    #torch.save(next_states_2nd_list, os.path.join(savedir, f'next_states_2nd_list.pt'))
