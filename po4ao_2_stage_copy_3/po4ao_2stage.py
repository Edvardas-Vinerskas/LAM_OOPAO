...
def step_2nd_stage(action):
    ...
    temp = ((prev_commands_2nd_stage * config['integrator']['leak']) + (action.squeeze())).clamp(-0.3, 0.3)
...
def run_episode_policy(past_obs, past_act, obs, replay, policy, sigma, episode_length, current_iteration):#, nxt_sts_2nd_lst, crnt_iter, ctrl_jit):
    ...
    for t in range(episode_length):
        ...
        timdeff_list.append(timedff)


    return reward_sum, past_obs, past_act, obs, timdeff_list
...
def main():
    ...
    directory_name = f'bench_1st200_2nd400_v9_atm_4_5_{timestamp}' #{timestamp}  #onsky_dubhe_1st200_2nd400_v2_wind{timestamp}
    savedir = f'~2026-04-17/{directory_name}'#{timestamp}'
    loaddir = f'~2026-04-17/saved'   # copy the models and replay buffers you want use here!!
    ...
    for i in range(iters):
        ...
        reward_sum, past_obs, past_act, obs, timdeff_ = run_episode_policy(past_obs, past_act, obs, replay, policy_copy_comp, sigma,
                                                                 episode_length, current_iter)#, next_states_2nd_list, current_iter, ctrl_jitter_list)
        
        timdeff_list_.append(timdeff_)
        start_2 = time.time()
    ...
    np.save(os.path.join(savedir, f"timdeff_list.npy"), np.asarray(timdeff_list_))
